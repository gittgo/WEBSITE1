        ...
        <div class="tab-con text-left">
          <div v-if="type===1">
            <div class="con-item"
                 ...
                 <span class="clock1">{{item.releaseDate}}</span>
              <div class="con-item-title" @click="goExpressDetails(item.id,'expressDetails')">{{item.title}}</div>
              <div class="con-item-con" @click="goExpressDetails(item.id,'expressDetails')">{{item.mainText}}</div>
              ...
              <div class="item-bottom cl">
                ...
                <div class="fl assis" @click="fabulous(index,item.id);">
                  <img src="../../../src/assets/images/topHand1.png" alt="" v-if="item.nocheck">
                  <img src="../../../src/assets/images/topHand2.png" alt="" v-if="item.check">
                  <template v-if="item.nocheck">
                    <span>利好</span>
                    <span>{{item.good}}</span>
                  </template>
                   <template v-if="item.check">
                    <span class="color1">利好</span>
                    <span class="color1">{{item.good}}</span>
                  </template>
                </div>
                <div class="fl assis" @click="noFabulous(index,item.id);">
                  <img src="../../../src/assets/images/botHand1.png" alt="" v-if="item.nochecked">
                  <img src="../../../src/assets/images/botHand2.png" alt="" v-if="item.checked">
                  <template v-if="item.nochecked">
                    <span>利空</span>
                    <span>{{item.bad}}</span>
                  </template>
                  <template v-if="item.checked">
                    <span class="color1">利空</span>
                    <span class="color1">{{item.bad}}</span>
                  </template>
                </div>
                <div class="rt share">
                  <el-popover
          ...
          <div v-else>
            <div class="con-item"
              ...
              <div class="con-item-con"
                   v-html="item.text" @click="goExpressDetails(item.id,'expressDetails')">{{item.text}}
              </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      fabulous:function(index,good){
        let _this = this;
        let params = {
          id:good
        };
        if(_this.data[index].check == 0){
          if(_this.data[index].checked == 0){
            _this.postData(_this.good,params,function(res){
              if(res.code == 0){
                _this.data[index].good = res.object.good;
                _this.data[index].check = 1;
                _this.data[index].nocheck = 0;
              }
            });
          }
      ...
      noFabulous:function(index,bad){
        ...
        if(_this.data[index].check == 0){
          if(_this.data[index].checked == 0){
            _this.postData(_this.bad,params,function(res){
              if(res.code == 0){
                _this.data[index].bad = res.object.bad;
                _this.data[index].checked = 1;
                _this.data[index].nochecked = 0;
              }
            });
          }
        }
