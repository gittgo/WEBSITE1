<template>
  <div v-if="busfastnews" >
      ...
      <div class="news text-left ">
        ...
        <div style="overflow-y: auto;height: 567px"
          ...
          <div class="new-item"
               ...
               v-if="index<=9"
               @click="goExpressDetails(item.id,'expressDetails')"
               :class="index===4?'news3':null"
               :key="index">
            <div style="line-height: 21px;" class="news-item-title">{{item.title}}
            </div>
            <div class="news-item-content omit">{{item.mainText}}</div>
    ...
    <div class="right-Ad-last relative home-news-bottom"  :class="!rightAd3?'border-line':null" >
...
.home-news img,.home-news-bottom img{
  transition: all 0.3s;
}
.home-news img:hover,.home-news-bottom img:hover{
  transform: scale(1.1);
}
.omit{
  position: relative; 
